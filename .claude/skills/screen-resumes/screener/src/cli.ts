...
async function writeResults(workspace: string, scored: ScoredResume[], jd: JD): Promise<void> {
...
program
  ...
  .action(async (opts) => {
    ...
    if (!existsSync(jdPath)) {
      ...
      console.error(
        `Copy the template: cp ~/.claude/skills/screen-resumes/references/jd-spec-template.yaml ${jdPath}`,
      );
      process.exit(1);
    }
